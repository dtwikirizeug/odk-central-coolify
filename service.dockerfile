...
RUN git describe --tags --dirty --always > /tmp/sentry-versions/central || echo "v2026.2.0" > /tmp/sentry-versions/central
WORKDIR /server
RUN git describe --tags --dirty --always > /tmp/sentry-versions/server || echo "v2026.2.0" > /tmp/sentry-versions/server
WORKDIR /client
RUN git describe --tags --dirty --always > /tmp/sentry-versions/client || echo "v2026.2.0" > /tmp/sentry-versions/client
